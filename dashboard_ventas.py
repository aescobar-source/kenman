...
st.set_page_config(
    ...
    page_icon="📊",
    layout="wide",
    initial_sidebar_state="collapsed",  # Sidebar only matters after file is loaded
)
...
# ─────────────────────────────────────────────
#  SIDEBAR — Branding + Filtros + Cambiar archivo
#  Aparece solo cuando ya hay datos cargados.
# ─────────────────────────────────────────────
with st.sidebar:
    # Branding
    st.markdown(f"""
    <div style='padding:16px 0 20px; border-bottom:1px solid rgba(255,255,255,0.15); margin-bottom:14px;'>
      <div style='font-size:1.1rem; font-weight:700; color:#FFFFFF; letter-spacing:-0.01em;'>📊 Ventas 2026</div>
      <div style='font-size:0.72rem; color:{COLORS["sidebar_sub"]}; margin-top:3px;'>Dashboard Ejecutivo</div>
    </div>
    """, unsafe_allow_html=True)

    # Cambiar archivo desde sidebar (una vez cargado)
    st.markdown(f"<div style='font-size:0.70rem;color:{COLORS['sidebar_sub']};font-weight:600;"
                f"text-transform:uppercase;letter-spacing:0.07em;margin-bottom:6px;'>"
                f"📂 Cambiar archivo</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div style='font-size:0.70rem;color:{COLORS['sidebar_sub']};font-weight:600;"
                f"text-transform:uppercase;letter-spacing:0.07em;margin-bottom:6px;margin-top:16px;'>"
