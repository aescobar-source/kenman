...
#  initial_sidebar_state="expanded" garantiza que el sidebar sea visible.
# ─────────────────────────────────────────────
st.set_page_config(
...
<style>
  ...
  /* ── Sidebar: abierto fija ancho, cerrado respeta animación nativa ── */
  ...
  section[data-testid="stSidebar"] * {{
    color: {COLORS['sidebar_txt']} !important;
  }}
  section[data-testid="stSidebar"] label,
  section[data-testid="stSidebar"] .stSelectbox label,
  section[data-testid="stSidebar"] .stMultiSelect label {{
    color: {COLORS['sidebar_sub']} !important;
  ...
  section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] {{
    ...
    border: 1.5px dashed {COLORS['sidebar_sub']} !important;
  ...
  section[data-testid="stSidebar"] button {{
    ...
    border: 1px solid {COLORS['sidebar_sub']} !important;
    color: {COLORS['sidebar_txt']} !important;
...
# ─────────────────────────────────────────────
#  JAVASCRIPT — ABRIR SIDEBAR AL PRIMER CARGUE
#  Usa MutationObserver para esperar el DOM real
#  y sessionStorage para no re-abrir si el usuario
#  decidió cerrarlo manualmente en esta sesión.
# ─────────────────────────────────────────────
st.markdown("""
<script>
(function() {
  var KEY = 'sb_user_closed';
  ...
  function openIfNeeded() {
    ...
    // Si el usuario NO lo cerró manualmente Y está colapsado → abrir
    if (expanded === 'false' && !sessionStorage.getItem(KEY)) {
  ...
  // Escuchar clic manual del usuario en el botón de colapso
  document.addEventListener('click', function(e) {
    var t = e.target;
    // Si el clic fue sobre el botón de colapso o su ícono hijo
    var collapseBtn = t.closest('[data-testid="collapsedControl"]') ||
                      t.closest('button[kind="header"]');
    if (collapseBtn) {
      var sidebar = document.querySelector('section[data-testid="stSidebar"]');
      if (sidebar && sidebar.getAttribute('aria-expanded') === 'true') {
        // Está abierto y el usuario lo está cerrando → marcar
        sessionStorage.setItem(KEY, '1');
      } else {
        // Lo está abriendo → borrar la marca
        sessionStorage.removeItem(KEY);
      }
    }
  }, true);

  // Intentar abrir en el arranque con MutationObserver
  var opened = false;
  var attempts = 0;
  var observer = new MutationObserver(function() {
    if (opened || attempts > 30) { observer.disconnect(); return; }
    attempts++;
    if (openIfNeeded()) {
      opened = true;
      observer.disconnect();
  ...
  // Fallbacks por tiempo
  setTimeout(function() { if (!opened) openIfNeeded(); }, 400);
  setTimeout(function() { if (!opened) openIfNeeded(); }, 1000);
  setTimeout(function() { if (!opened) { openIfNeeded(); observer.disconnect(); } }, 2000);
})();
</script>
""", unsafe_allow_html=True)

# ─────────────────────────────────────────────
#  SIDEBAR — SIEMPRE VISIBLE, PERSISTENTE
#  El sidebar se construye completo ANTES del st.stop()
#  para garantizar su renderizado incluso sin archivo.
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown(f"""
    <div style='padding:18px 0 22px; border-bottom:1px solid rgba(255,255,255,0.15); margin-bottom:16px;'>
      <div style='font-size:1.15rem; font-weight:700; color:#FFFFFF; letter-spacing:-0.01em;'>📊 Ventas 2026</div>
      <div style='font-size:0.73rem; color:{COLORS["sidebar_sub"]}; margin-top:4px;'>Dashboard Ejecutivo</div>
    </div>
    """, unsafe_allow_html=True)

    uploaded_file = st.file_uploader(
        "📂 Subir BD de Ventas (Excel)",
        type=["xlsx", "xls"],
        help="Carga tu archivo Excel con las hojas BD de ventas y META.",
    )

    if uploaded_file is None:
        st.markdown(f"""
        <div style='margin-top:20px; padding:14px; background:rgba(255,255,255,0.07);
             border-radius:8px; border:1px solid rgba(255,255,255,0.12);'>
          <div style='font-size:0.78rem; color:{COLORS["sidebar_sub"]}; font-weight:600;
               text-transform:uppercase; letter-spacing:0.06em; margin-bottom:8px;'>
            Estado
          </div>
          <div style='font-size:0.85rem; color:{COLORS["sidebar_txt"]};'>
            ⚠️ Esperando archivo de datos…
          </div>
        </div>
        """, unsafe_allow_html=True)
...
if uploaded_file is None:
    ...
    <div class="welcome-box">
      ...
      <div class="welcome-title">Dashboard Ejecutivo de Ventas</div>
      <div class="welcome-body">
        Para comenzar, sube tu archivo <strong>Excel de ventas</strong> usando el panel izquierdo.<br><br>
        El archivo debe contener una hoja <code>BD</code> con los registros de venta
        y opcionalmente una hoja <code>META</code> con las metas por ejecutivo.
      </div>
      <div class="welcome-arrow">← Usa el panel izquierdo para cargar el archivo</div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
    ...
    try:
        ...
        if sheet_metas:
            ...
            if "META_TOTAL" in raw.columns:
                ...
                # Adjuntar al df_metas para uso posterior
                df_metas._raw_meta = raw

        return df, df_metas, sheet_metas, None

    except Exception as e:
        return None, pd.DataFrame(), None, f"Error al procesar los datos: {e}"


# ── Cargar datos con manejo de error ─────────────────────────────────────────
result = load_data(uploaded_file.getvalue())
df, df_metas, sheet_metas_nombre, load_error = result
...
# ─────────────────────────────────────────────
#  SIDEBAR — FILTROS (solo cuando hay datos)
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown(f"<div style='font-size:0.72rem;color:{COLORS['sidebar_sub']};font-weight:600;"
                f"text-transform:uppercase;letter-spacing:0.07em;margin-bottom:10px;margin-top:8px;'>"
...
# ═══════════════════════════════════════════════════════════
#  SECCIÓN: CUMPLIMIENTO JERÁRQUICO
...
if has_metas:
    ...
    try:
        if "SEGMENTO" in dff.columns:
            ...
            raw_meta = getattr(df_metas, "_raw_meta", None)
