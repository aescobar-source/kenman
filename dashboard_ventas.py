...
<style>
  ...
  /* ── Sidebar: fuerza apertura en primer render, pero permite colapsar ── */
  /* Expandido: ancho fijo */
  section[data-testid="stSidebar"][aria-expanded="true"] {{
    background-color: {COLORS['accent']} !important;
    min-width: 260px !important;
    max-width: 320px !important;
  }}
  /* Colapsado: deja que Streamlit lo maneje (no bloqueamos) */
...
# ─────────────────────────────────────────────
#  JAVASCRIPT — ABRIR SIDEBAR SOLO EN PRIMER RENDER
#  Solo se ejecuta UNA vez al cargar la página.
#  Después, el usuario puede colapsar/expandir libremente.
# ─────────────────────────────────────────────
st.markdown("""
<script>
(function openSidebarOnce() {
  // Clave de sesión para no re-abrir si el usuario lo cerró
  var SESSION_KEY = 'st_sidebar_opened';
  ...
  function tryOpen() {
    ...
    var alreadyOpened = sessionStorage.getItem(SESSION_KEY);

    if (isCollapsed && !alreadyOpened) {
      // Buscar el botón de apertura (collapsedControl aparece cuando está cerrado)
      ...
      // Ya está abierto — marcar para no volver a forzar
      sessionStorage.setItem(SESSION_KEY, '1');
    }
  }

  // Solo intentar en el arranque inicial, con delays para esperar el DOM
