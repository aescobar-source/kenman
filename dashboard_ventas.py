...
try:
    if uploaded_file.name.endswith('.csv'):
        ...
        if sheet_metas:
            ...
            # Homologar columnas
            if "EJECUTIVO" in df_metas.columns:
                df_metas.rename(columns={"EJECUTIVO": "EJECUTVO"}, inplace=True)
            # Buscar columna de meta
            meta_cols = [c for c in df_metas.columns if 'META' in c.upper() and c.upper() != 'MARCA']
            if meta_cols:
...
# ─────────────────────────────────────────────
if not dff_metas.empty and "EJECUTVO" in dff.columns:
