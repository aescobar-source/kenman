...
# Escala de grises para gráficos
...
# ─────────────────────────────────────────────
#  CARGA DE DATOS (solo si hay archivo)
# ─────────────────────────────────────────────
...
try:
    if uploaded_file.name.endswith('.csv'):
        ...
        df_metas = pd.DataFrame()  # No hay metas en CSV
    else:
        # Leer todas las hojas del Excel
        xls = pd.ExcelFile(uploaded_file)
        sheet_names = xls.sheet_names

        # Identificar hoja de ventas (BD o VENTAS)
        sheet_ventas = next(
...
# ─────────────────────────────────────────────
#  GRÁFICO: CUMPLIMIENTO POR EJECUTIVO (SOLO SI EXISTEN DATOS)
# ─────────────────────────────────────────────
if not dff_metas.empty and "EJECUTVO" in dff.columns and "META_TOTAL" in dff_metas.columns:
    ...
    # Asegurar que ambos DataFrames tengan la columna de ejecutivo
...
# ─────────────────────────────────────────────
#  CÁLCULO DE KPIs PRINCIPALES (para el resto del dashboard)
# ─────────────────────────────────────────────
...
mes_util = dff.groupby("MES")["UTILIDAD TOTAL $"].sum() if "MES" in dff.columns and "UTILIDAD TOTAL $" in dff.columns else pd.Series()
...
with tab6:
    ...
    with col4:
        ...
        fig7.update_layout(barmode="stack", xaxis_tickangle=-25,
                           legend=dict(orientation="h", y=-0.25))
