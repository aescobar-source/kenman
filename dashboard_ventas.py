...
<style>
  ...
  /* ── Global KPI Cards (Rendimiento Global) ── */
  .gkpi-row {{
    display: flex;
    gap: 16px;
    margin-bottom: 8px;
  ...
  .gkpi-delta-pos {{
    ...
    font-weight: 600;
    color: {COLORS['pos']};
    background: rgba(13,110,59,0.08);
    border-radius: 20px;
    padding: 3px 10px;
    margin-top: 2px;
  }}
...
if meta_prorrata > 0:
    ...
    delta1_cls   = "gkpi-delta-pos" if desviacion >= 0 else "gkpi-delta-neg"
    delta1_icon  = "▲" if desviacion >= 0 else "▼"
    delta1_txt   = f"{delta1_icon} {signo}{desviacion:.1f}% vs Meta &nbsp;·&nbsp; {cumplimiento_pct:.1f}% lograda"
else:
    delta1_cls   = "gkpi-delta-neu"
    delta1_txt   = "Sin meta asignada"

if meta_prorrata > 0:
    falta        = meta_prorrata - total_venta
    delta2_cls   = "gkpi-delta-neg" if falta > 0 else "gkpi-delta-pos"
    delta2_icon  = "▼" if falta > 0 else "✅"
    delta2_txt   = f"{delta2_icon} Falta {fmt_usd(max(falta,0))} para meta" if falta > 0 else "✅ Meta superada"
    meta2_label  = f"Meta Período ({n_meses_sel}m)"
    meta2_value  = fmt_usd(meta_prorrata)
else:
    delta2_cls   = "gkpi-delta-neu"
    delta2_txt   = "Meta no disponible"
    meta2_label  = "Meta Anual Total"
    meta2_value  = fmt_usd(total_meta_anual) if total_meta_anual > 0 else "N/A"

margen_cls  = "gkpi-delta-pos" if margen_global >= 0.25 else "gkpi-delta-warn"
margen_icon = "▲" if margen_global >= 0.25 else "▼"

st.markdown(f"""
<div class="gkpi-row">

  <div class="gkpi-card">
    <div class="gkpi-label">Venta Real (período)</div>
    <div class="gkpi-value">{fmt_usd(total_venta)}</div>
    <span class="{delta1_cls}">{delta1_txt}</span>
  </div>

  <div class="gkpi-card">
    <div class="gkpi-label">{meta2_label}</div>
    <div class="gkpi-value">{meta2_value}</div>
    <span class="{delta2_cls}">{delta2_txt}</span>
  </div>

  <div class="gkpi-card">
    <div class="gkpi-label">Margen Global</div>
    <div class="gkpi-value">{margen_global*100:.1f}%</div>
    <span class="{margen_cls}">{margen_icon} Mejor mes: {best_mes.title()}</span>
  </div>

  <div class="gkpi-card">
    <div class="gkpi-label">Utilidad Bruta</div>
    <div class="gkpi-value">{fmt_usd(total_utilidad)}</div>
    <span class="gkpi-delta-neu">Margen: {fmt_pct(margen_global)}</span>
  </div>
...
with tab1:
    # ═══════════════════════════════════════════════════════════
    #  CUMPLIMIENTO JERÁRQUICO — dentro de tab1
