...
GRAY_SCALE = ["#4A4A4A", "#6B6B6B", "#8A8A8A", "#A8A8A8", "#C8C8C8", "#E0E0E0", "#F0F0F0"]

CITY_COORDS = {
    "MANAGUA": (12.1364, -86.2514),
    "LEON": (12.4379, -86.8780),
    "GRANADA": (11.9299, -85.9560),
    "MASAYA": (11.9743, -86.0942),
    "CHINANDEGA": (12.6296, -87.1265),
    "MATAGALPA": (12.9258, -85.9170),
    "ESTELI": (13.0918, -86.3538),
    "JINOTEGA": (13.0911, -86.0000),
    "NUEVA SEGOVIA": (13.6314, -86.4757),
    "RIO SAN JUAN": (11.1403, -84.7737),
    "RAAS": (12.1199, -84.2297),
    "RIVAS": (11.4365, -85.8279),
    "CARAZO": (11.8446, -86.1983),
    "BOACO": (12.4714, -85.6597),
    "CHONTALES": (12.0795, -85.4049),
    "MADRIZ": (13.4766, -86.5835),
}
...
def chart_layout(fig, title="", height=380):
    fig.update_layout(
        title=dict(
            text=title,
            font=dict(size=13, color=COLORS["text_dark"], family="Inter"),
            x=0,
            xanchor="left"
        ),
...
def ensure_columns(df, required_cols, default_value=0):
    """Asegura que las columnas requeridas existan en el DataFrame, creándolas con valor por defecto."""
...
# ─────────────────────────────────────────────
#  SIDEBAR — SUBIDA DE ARCHIVO Y FILTROS
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown("""
    ...
    uploaded_file = st.file_uploader("📂 Sube tu archivo de ventas", type=["xlsx", "xls", "csv"])
...
try:
    if uploaded_file.name.endswith('.csv'):
        ...
        sheet_ventas = next(
            (s for s in sheet_names if 'BD' in s.upper() or 'VENTA' in s.upper()),
            sheet_names[0]
        )
        sheet_metas = next(
            (s for s in sheet_names if 'META' in s.upper()),
            None
        )
        df = pd.read_excel(xls, sheet_name=sheet_ventas)
        df_metas = pd.DataFrame()
        if sheet_metas:
            df_metas = pd.read_excel(xls, sheet_name=sheet_metas)
            df_metas.columns = df_metas.columns.str.strip()
            meta_cols = [c for c in df_metas.columns if 'META' in c.upper()]
            if meta_cols:
                df_metas.rename(columns={meta_cols[0]: "META_TOTAL"}, inplace=True)
            ejec_cols = [c for c in df_metas.columns if 'EJECUT' in c.upper()]
            if ejec_cols:
                df_metas.rename(columns={ejec_cols[0]: "EJECUTVO"}, inplace=True)
...
# ─────────────────────────────────────────────
#  LIMPIEZA Y ESTANDARIZACIÓN DE LA BD PRINCIPAL
# ─────────────────────────────────────────────
df.columns = df.columns.str.strip()

# Columnas que deben existir (se crean con 0 si faltan)
columnas_requeridas = [
    "TOTAL VENTA", "UTILIDAD TOTAL $", "TOTAL DESCUENTO", "COSTO BODEGA TOTAL",
    "PARES", "FACTURA_KEY", "SUBTOTAL", "% DE DESCUENTO"
]
...
# Ordenar meses si existe la columna MES
...
# Costo total
if "COSTO BODEGA TOTAL" in df.columns:
    df["COSTO_TOTAL"] = df["COSTO BODEGA TOTAL"].fillna(0)
else:
    df["COSTO_TOTAL"] = 0

# Descuento booleano
if "TOTAL DESCUENTO" in df.columns:
    df["TIENE_DESCUENTO"] = df["TOTAL DESCUENTO"] > 0
else:
    df["TIENE_DESCUENTO"] = False
...
with st.sidebar:
    ...
    meses_disp = sorted(
        df["MES"].unique(),
        key=lambda x: MES_ORDER.get(x, 99)
...
if sel_zona:
    mask &= df["ZONA"].isin(sel_zona)
if sel_seg:
    mask &= df["SEGMENTO"].isin(sel_seg)
if sel_marca:
    mask &= df["MARCA"].isin(sel_marca)
if sel_prod:
    mask &= df["TIPO DE PRODUCTO"].isin(sel_prod)
if sel_ejec:
    mask &= df["EJECUTVO"].isin(sel_ejec)

dff = df[mask].copy()

# Filtrar metas si existen
if not df_metas.empty:
    mask_metas = pd.Series(True, index=df_metas.index)
    if sel_mes and "MES" in df_metas.columns:
        mask_metas &= df_metas["MES"].isin(sel_mes)
    if sel_seg and "SEGMENTO" in df_metas.columns:
        mask_metas &= df_metas["SEGMENTO"].isin(sel_seg)
    if sel_marca and "MARCA" in df_metas.columns:
        mask_metas &= df_metas["MARCA"].isin(sel_marca)
    if sel_ejec and "EJECUTVO" in df_metas.columns:
        mask_metas &= df_metas["EJECUTVO"].isin(sel_ejec)
    dff_metas = df_metas[mask_metas].copy()
else:
...
total_ventas = dff["TOTAL VENTA"].sum()
...
# ─────────────────────────────────────────────
#  GRÁFICO: CUMPLIMIENTO POR EJECUTIVO (solo si hay datos)
# ─────────────────────────────────────────────
if not dff_metas.empty and "EJECUTVO" in dff.columns and "META_TOTAL" in dff_metas.columns:
    ...
    df_comp["CUMPLIMIENTO_%"] = np.where(
        df_comp["META_TOTAL"] > 0,
        (df_comp["TOTAL VENTA"] / df_comp["META_TOTAL"]) * 100,
        0
    )
    ...
    st.info("ℹ️ No se encontraron datos de metas para mostrar el gráfico de cumplimiento por ejecutivo.")
...
subtotal = dff["SUBTOTAL"].sum() if "SUBTOTAL" in dff.columns else 0
descuento_rate = total_descuento / subtotal if subtotal > 0 else 0
efect_desc = dff["TIENE_DESCUENTO"].mean() if "TIENE_DESCUENTO" in dff.columns else 0

# Mes con mejor utilidad
mes_util = dff.groupby("MES")["UTILIDAD TOTAL $"].sum() if "MES" in dff.columns else pd.Series()
...
with tab1:
    ...
    # Solo si existe la columna MES y hay datos
    if "MES" in dff.columns and not dff.empty:
        ...
        st.warning("No hay datos mensuales para mostrar tendencias.")
...
with tab4:
    ...
    if "EJECUTVO" in dff.columns:
        ...
        st.warning("No se encontró la columna 'EJECUTVO' para analizar ejecutivos.")
...
with tab5:
    ...
    if "TIPO DE VENTA" in dff.columns and "MES" in dff.columns:
...
with tab6:
    ...
    if "MES" in dff.columns:
        ...
        # Solo columnas que existen en dff
        num_cols_exist = [c for c in num_cols if c in dff.columns]
        if len(num_cols_exist) > 1:
            corr = dff[num_cols_exist].corr()
            labels = [c.replace("TOTAL ", "").replace(" $", "") for c in num_cols_exist]
        ...
        if "% DE DESCUENTO" in dff.columns:
            ...
            with col3:
                ...
                chart_layout(fig6, "Rango de Descuento vs Margen%: ¿a qué punto el descuento destruye margen?", height=380)
                fig6.update_layout(legend=dict(orientation="h", y=-0.2))
                st.plotly_chart(fig6, use_container_width=True)

        # Tabla variaciones MoM
        if "MES" in dff.columns and not mes_var.empty:
    ...
    # Insights automáticos (solo si existen las columnas necesarias)
    if "ZONA" in dff.columns and "UTILIDAD TOTAL $" in dff.columns:
        ...
        mejor_marca = dff.groupby("MARCA")["UTILIDAD TOTAL $"].sum().idxmax() if "MARCA" in dff.columns else "N/A"
        mejor_ejec = dff.groupby("EJECUTVO")["TOTAL VENTA"].sum().idxmax() if "EJECUTVO" in dff.columns else "N/A"
        mayor_desc_ejec = dff.groupby("EJECUTVO")["TOTAL DESCUENTO"].sum().idxmax() if "EJECUTVO" in dff.columns and "TOTAL DESCUENTO" in dff.columns else "N/A"
