...
<style>
  ...
  /* ══════════════════════════════════════════
     SIDEBAR FORCE-OPEN
     Streamlit Cloud a veces colapsa el sidebar
     ignorando initial_sidebar_state="expanded".
     Este bloque lo mantiene siempre visible.
     ══════════════════════════════════════════ */

  /* 1. Fuerza el ancho del sidebar y lo muestra */
  ...
  /* 2. Elimina la clase "collapsed" que Streamlit añade */
  ...
  /* 3. Oculta el botón hamburguesa que colapsa el sidebar */
  button[data-testid="collapsedControl"],
  button[kind="header"] {{
    display: none !important;
  }}

  /* 4. Ajusta el área principal para no solapar con sidebar */
...
# ─────────────────────────────────────────────
#  JAVASCRIPT — FORZAR SIDEBAR ABIERTO
#  Streamlit Cloud colapsa el sidebar en el primer
#  render. Este script detecta si está cerrado y
#  simula el clic en el botón de apertura.
...
(function openSidebar() {
  function tryOpen() {
    // Selector del botón que abre el sidebar cuando está colapsado
    var btn = document.querySelector('[data-testid="collapsedControl"]');
    if (btn) {
      btn.click();
      return;
    }
    // Alternativa: sidebar con aria-expanded="false"
  ...
  // Intentar inmediatamente y con delay para garantizar que el DOM esté listo
